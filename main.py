...
def start_game(mode):
    ...
    try:
        ...
        y = 32
    if mode == 0:
        name_1 = input_text("Введите имя 1 игрока:")
        if len(name_1) == 0:
            name_1 = "Игрок 1"
        name_2 = input_text("Введите имя 2 игрока:")
        if len(name_2) == 0:
            name_2 = "Игрок 2"
        ...
        show_result(score, mode, 0, name_1, name_2)
    if mode == 1:
        ...
        level = min(1, level)
        level = max(0, level)
        player = input_text("Кто ходит первый? {0 или 1}")
        try:
            player = int(player)
        except ValueError:
            player = 0
        ...
        if player == 0:
            ...
            show_result(score, mode, 1, name_2=name)
    if mode == 2:
        game = Game(x, y, 2)
        score = game.start()
        show_result(score, mode, 0, "Синие", "Красные")
    build_menu()
...
def show_result(score, mode, player=0, name_1=None, name_2=None):
    ...
    while True:
        ...
        if mode != 1:
            ...
            if player == 0:
                text = f"{name_1}: {score[0]}"
                text_score_1 = courier.render(text, 0, white)
                text = f"Computer: {score[1]}"
                text_score_2 = courier.render(text, 0, white)
            else:
                text = f"Computer: {score[0]}"
                text_score_1 = courier.render(text, 0, white)
                text = f"{name_2}: {score[1]}"
                text_score_2 = courier.render(text, 0, white)
...
def start():
    ...
    menu.add.button('PVC', lambda: start_game(1))
